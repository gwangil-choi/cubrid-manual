...
*   client: <DB 사용자>@<응용 클라이언트 호스트 명>|<프로그램 이름>(<프로세스 ID>)
*   sql: :red:`일시적 볼륨이 필요한 SQL. INSERT ... SELECT를 제외한 모든 INSERT 문, DDL 문 등은 DB 서버에 SQL이 전달되지 않기 때문에 EMPTY로 표시된다.` 
    :red:`SELECT, UPDATE, DELETE 문은 SQL이 표시된다.`
*   bind: :red:`바인딩된 값`
*   time: :red:`일시적 볼륨 생성에 소요된 시간(ms)`
*   pages: :red:`일시적 볼륨 생성에 필요한  페이지의 개수` 
...
:red:`cubrid broker status에서 사용하는 [options]는 다음과 같다. 이들 중 -b, -q, -c, -m, -S, -P, -f는 출력할 정보를 정의하는 모니터링 옵션이고, -s, -l, -t는 출력을 제어하는 옵션이다. 이 모든 옵션들은 서로 조합하여 사용하는 것이 가능하다.`
...
*   ERR-Q: :red:`에러가 발생한 질의의 개수. 옵션이 "-b -s <sec>"인 경우 -s 옵션으로 지정한 초 동안의 에러 개수로 매번 갱신됨.`
...
*   CANCELED: :red:`브로커 시작 이후 사용자 인터럽트로 인해 취소된 질의의 개수 (-l N 옵션과 함께 사용하면 N초 동안 누적된 개수).`
