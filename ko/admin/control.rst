...
임시 볼륨(temporary volume)이 확장되면 해당 시각을 기록한다. 이를 통해 임시 볼륨 확장을 유발한 트랜잭션을 확인할 수 있다.
...
*   client: <DB 사용자>@<응용 클라이언트 호스트 명>|<프로그램 이름>(<프로세스 ID>)
*   sql: :red:`임시 데이터를 위한 공간이 더 필요한 SQL이다. INSERT ... SELECT 구문을 제외한 모든 SQL 질의문과 DDL 질의문은 DB 서버로 전달되지 않으므로 EMPTY로 표시된다. `
    :red:`이 항목에는 SELECT, UPDATE 및 DELETE 질의문이 표시된다.`
*   bind: :red:`바인딩 값`
*   time: :red:`일시적 볼륨 생성에 필요한 시간(ms)`
*   pages: :red:`새 일시적 볼륨 내에서 사용할 수 있는 페이지의 수` 
...
:red:`다음 [옵션]은 **cubrid broker status** 유틸리티로 이용할 수 있다. -b, -q, -c, -m, -S, -P 및 -f는 출력할 정보를 정의하는 옵션이며, -s, -l 및 -t는 출력을 제어하는 옵션이다. 이 모든 옵션은 서로 결합해서 사용할 수 있다.`
...
*   ERR-Q: :red:`오류가 발견된 질의 수이다. "-b -s <sec>" 옵션이 있는 경우 이 옵션으로 지정된 시간(초) 동안 발생한 오류 수가 매번 갱신된다.`
...
*   CANCELED: :red:`브로커가 시작된 후 사용자가 취소한 질의 수이다(**-l** *N* 옵션과 함께 사용할 경우 *N*초 동안 누적된 수를 명시한다).`
