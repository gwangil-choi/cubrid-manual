...
*value* :red:`파라미터명에 **DEFAULT** 값을 주면 **call_stack_dump_activation_list** 파라미터를 제외하고 해당 파라미터를 기본값으로 재설정한다.`
...
**temp_file_memory_size_in_pages**
    ...
    *   :red:`일시적 데이터 저장을 위한 영구적 볼륨`
    *   :red:`일시적 볼륨`
    
    :red:`기존의 공간이 소진되면 일시적 결과 저장을 위해 캐시 버퍼 -> 영구적 볼륨 -> 일시적 볼륨의 순서로 공간이 사용된다.`
...
**db_volume_size**
    ...
    *   :red:`데이터베이스 볼륨 공간을 모두 사용하면 자동으로 추가되는 볼륨의 기본 크기`
 ...
 **data_buffer_neighbor_flush_nondirty** :red:`인접 플러시 기능이 활성화상태이고(**data_buffer_neighbor_flush_pages**가 1보다 큰 경우) 희생자 후보 페이지가 플러시될 때 , 더티 페이지에 인접한 더티가 아닌 단일 페이지들도 함께 플러시된다.`
