...
cubrid 유틸리티의 사용법(구문)은 다음과 같다. ::
    ...
    utility_name :
        ...
        gen_tz [option] [<database-name>]  --- :red:'공유 라이브러리로 컴파일할 수 있는 타임존 데이터가 포함된 C 소스 파일 생성'
        dump_tz [option]  --- :red:'타임존 관련 정보 출력'
...
.. option:: --db-volume-size=SIZE
    ...
    :red:`이 옵션은 생성될 데이터베이스 볼륨의 크기를 지정한다. 기본값은 시스템 파라미터 **db_volume_size** 에 지정된 값이다. 단위는 K, M, G 및 T로 설정할 수 있으며, 각각 킬로바이트(KB), 메가바이트(MB), 기가바이트(GB) 및 테라바이트(TB)를 나타낸다. 단위를 생략하면 바이트 단위가 적용된다. 데이터베이스의 크기는 항상 64개 디스크 섹터로 올림된다. 이 값은 페이지의 크기에 따라 달라질 수 있으며, 페이지 크기가 각각 4k, 8k 및 16k인 경우 16M, 32M 또는 64M이 될 수 있다.`
...
.. option:: --log-volume-size=SIZE 
    ...
    :red:`이 옵션은 데이터베이스 로그 볼륨의 크기를 지정한다. 기본값은 데이터베이스 볼륨 크기와 같으며, 최소값은 20M이다. 단위는 K, M, G 및 T로 설정할 수 있으며, 각각 킬로바이트(KB), 메가바이트(MB), 기가바이트(GB) 및 테라바이트(TB)를 나타낸다. 단위를 생략하면 바이트 단위가 적용된다.`
...
:red:`CUBRID 볼륨을 세부적으로 관리하려면 addvoldb 를 사용하면 된다. 각 볼륨 파일명, 경로, 용도 및 크기를 세분해서 관리할 수 있다. 데이터베이스 시스템은 모든 볼륨을 직접 관리할 수 있으나, 옵션을 생략하면 각각의 새 볼륨을 구성할 때는 기본값을 사용한다`
...
.. option:: --db-volume-size=SIZE
    ...
    **--db-volume-size** :red:`지정한 데이터베이스에 추가될 볼륨 크기를 지정하는 옵션이다. **--db-volume-size** 옵션을 생략하면 시스템 파라미터 **db_volume_size**의 값이 기본값으로 사용된다. 단위는 K, M, G 및 T로 설정할 수 있으며, 각각 킬로바이트(KB), 메가바이트(MB), 기가바이트(GB) 및 테라바이트(TB)를 나타낸다. 단위를 생략하면 바이트 단위가 적용된다. 데이터베이스의 크기는 항상 64개 디스크 섹터로 올림된다. 이 값은 페이지의 크기에 따라 달라질 수 있으며, 페이지 크기가 각각 4k, 8k 및 16k인 경우 16M, 32M 또는 64M이 될 수 있다`
...
.. option:: --comment=COMMENT
    ...
    :red:`다음은  볼륨을 추가하고 추가 정보로 주석을 삽입하는 구문이다.` ::
...
.. option:: -p, --purpose=PURPOSE
    ...
    * **TEMPORARY DATA** :red:`: 질의 처리 및 정렬을 수행할 때 중간 결과와 최종 결과 저장.`
    ...
    :red:`이 옵션이 지정되지 않은 경우 해당 볼륨의 용도는 기본적으로 **PERMANENT DATA** 로 간주한다. 다음은 임시 데이터 용도로 볼륨을 추가하는 구문이다.` ::
...
.. option:: -s, --summarize
    ...
    :red:`이 옵션은 볼륨 타입 및 용도별로 볼륨 수, 사용된 크기, 빈 공간 크기 및 총 공간 크기를 집계한다. 볼륨에는 영구적 데이터를 사용하는 영구적 볼륨, 일시적 데이터를 사용하는 영구적 볼륨과 일시적 데이터를 사용하는 일시적 볼륨, 이 세 가지 종류가 있으며 영구적 데이터를 사용하는 일시적 볼륨은 없다.  마지막 행은 모든 볼륨 타입의 총 공간 값을 출력한다.` ::
...
.. option:: -p, --purpose
    ...
    :red:`이 옵션은 저장된 데이터의 용도에 대한 자세한 정보를 출력한다. 이 정보에는 파일 수, 사용된 크기, 파일 테이블 크기, 예약된 섹터 크기 및 총 크기가 포함된다.` ::
...
*   **statdump**: :red:`데이터베이스 서버 수행 시 통계 정보를 출력하는 명령어이다.`
...
.. option:: -i, --interval=SECOND
    ...
    *  :red:`Accumulator: 트래킹이 발생할 때마다 통계값이 증가한다.`
    ...
    *  :red:`Complex: 통계는 다양한 속성으로 구분 된 작업에 대한 여러 값을 추적한다.`
    ...
    :red:`대부분의 통계는 트래킹이 발생하면 증가하는 Accumulator 통계이다. 나머지는 작업 수와 지속 시간을 모두 추적하는 Counter/timer 통계, 데이터베이스에서 수집된 Snapshot 통계 및 다른 값을 기반으로 계산된(Computed) 통계이다. 마지막으로, 일부 작업에 대한 자세한 정보를 추적하는 복합(Complex) 통계가 있다.` 
    ...
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | ..bt_find_unique                         | Counter/timer  | B-tree 의 'find-unique' 수행 시간과 횟수                              |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | ..btrange_search                         | Counter/timer  | B-tree 의 'range-search' 수행 시간과 횟수                             |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | ..bt_insert_obj                          | Counter/timer  | B-tree 의 'insert-object' 수행 시간과 횟수                            |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | ..bt_delete_obj                          | Counter/timer  | B-tree 의 'physical delete object' 수행 시간과 횟수                   |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | ..bt_mvcc_delete                         | Counter/timer  | B-tree 의 'mvcc delete' 수행 시간과 횟수                              |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | ..bt_mark_delete                         | Counter/timer  | B-tree 의 'mark delete' 수행 시간과 횟수                              |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | ..bt_undo_insert                         | Counter/timer  | B-tree 의 'undo insert' 수행 시간과 횟수                              |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | ..bt_undo_delete                         | Counter/timer  | B-tree 의 'undo physical delete' 수행 시간과 횟수                     |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | ..bt_undo_mvcc_delete                    | Counter/timer  | B-tree 의 'undo mvcc delete' 수행 시간과 횟수                         |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | ..bt_vacuum                              | Counter/timer  | B-tree 의 'vacuum deleted object' 수행 시간과 횟수                    |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | ..bt_vacuum_insid                        | Counter/timer  | B-tree 의 'insert id' 연산에 대한 vacuum 작업의 수행 시간과 횟수      |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    ...
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | ..bt_traverse                            | Counter/timer  | B-tree 순회의 수행 시간과 횟수                                        |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | ..bt_find_unique_traverse                | Counter/timer  | B-tree 'find unique' 연산을 위한 B-tree 순회의 수행 시간과 횟수       |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | ..bt_range_search_traverse               | Counter/timer  | B-tree 'range search' 순회의 수행 시간과 횟수                         |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | ..bt_insert_traverse                     | Counter/timer  | B-tree 'insert' 순회의 수행 시간과 횟수                               |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | ..bt_delete_traverse                     | Counter/timer  | B-tree 'physical delete' 순회의 수행 시간과 횟수                      |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | ..bt_mvcc_delete_traverse                | Counter/timer  | B-tree 'mvcc delete' 순회의 수행 시간과 횟수                          |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | ..bt_mark_delete_traverse                | Counter/timer  | B-tree 'mark delete' 순회의 수행 시간과 횟수                          |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | ..bt_undo_insert_traverse                | Counter/timer  | B-tree 'undo physical insert' 순회의 수행 시간과 횟수                 |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | ..bt_undo_delete_traverse                | Counter/timer  | B-tree 'undo physical delete' 순회의 수행 시간과 횟수                 |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | ..bt_undo_mvcc_delete_traverse           | Counter/timer  | B-tree 'undo delete' 순회의 수행 시간과 횟수                          |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | ..bt_vacuum_traverse                     | Counter/timer  | B-tree 'vacuum deleted object' 순회의 수행 시간과 횟수                |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | ..bt_vacuum_insid_traverse               | Counter/timer  | B-tree 'vacuum insert id' 순회의 수행 시간과 횟수                     |
    +------------------+------------------------------------------+----------------+-----------------------------------------------------------------------+
    ...
    | 관련             |                                          |                |                                                                       |
    +------------------+------------------------------------------+----------------+-----------------------------------------------------------------------+
    | 힙 관련          | Num_heap_stats_bestspace_entries         | Accumulator    | 최적합 페이지 목록에 저장된 적합 페이지 개수                          |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    ...
    |                  |                                          |                | | (*)                                                                 |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | Num_heap_rel_to_rel_updates              | Accumulator    | | 비 MVCC 모드에서 힙 재배치타입에서 재배치타입으로 갱신된 레코드의   |
    |                  |                                          |                | | 횟수(*)                                                             |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    |                  | Num_heap_rel_to_big_updates              | Accumulator    | | 비 MVCC 모드에서 BIG 유형 레코드에 대한 힙 RELOCATION의 갱신 횟수   |
    |                  |                                          |                | | (*)                                                                 |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    ...
    +------------------+------------------------------------------+----------------+-----------------------------------------------------------------------+
    | 질의 계획 캐시   | Num_plan_cache_add                       | Accumulator    | 쿼리 캐시 엔트리가 새로  시작된  횟수                                 |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    ...
    +------------------+------------------------------------------+----------------+-----------------------------------------------------------------------+
    | Vacuuming 관련   | Num_vacuum_log_pages_vacuumed            | Accumulator    | 회수 워커에 의해 처리된 데이터 페이지 로그 횟수                       |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    ...
    +------------------+------------------------------------------+----------------+-----------------------------------------------------------------------+
    | 페이지 버퍼 수정 | Data_page_fix_lock_acquire_time_msec     | Computed       | 디스크에서 페이지를 로드하는 트랜잭션이 기다리는 시간                 |
    |                  +------------------------------------------+----------------+-----------------------------------------------------------------------+
    ...
    |                  |                                          |                | | - 성공/실패 프로모션                                                |
    +------------------+------------------------------------------+----------------+-----------------------------------------------------------------------+
    | 페이지 버퍼      | Num_unfix_void_to_private_top            | Accumulator    | 수정되지 않고 새로 로드된 데이터 페이지 및 내부 LRU 맨 위에 추가된 수 |
    | | 비수정         +------------------------------------------+----------------+-----------------------------------------------------------------------+
    ...
    |                  |                                          |                | | - 홀더 래치 모드                                                    |
    +------------------+------------------------------------------+----------------+-----------------------------------------------------------------------+
    | 페이지 버퍼 I/O  | Data_page_buffer_hit_ratio               | Computed       | | 데이터 페이지 버퍼의 히트 비율                                      |
    ...
    |                  |                                          |                | | 플러시 스레드를 생략한 BCB의 수                                     |
    +------------------+------------------------------------------+----------------+-----------------------------------------------------------------------+
    | | 페이지 버퍼    | ..alloc_bcb                              | Counter/timer  | | 새로운 데이터 페이지를 저장하기위한 BCB 할당의 수와 기간.           |
    ...
    +------------------+------------------------------------------+----------------+-----------------------------------------------------------------------+
    | MVCC 스냅샷      | Time_get_snapshot_acquire_time:          | Accumulator    | 스냅샷을 얻기 위해 모든 트랜잭션이 소비 한 총 시간                    |
    | 관련             +------------------------------------------+----------------+-----------------------------------------------------------------------+
...
.. note::
    ...
    :red:`아래는 활성화/비활성화 할 수 있는 성능 통계 세트 목록이다. 각 세트의 값은 2의 제곱으로 표현된다.  이 세트들은 **extended_statistics_activation** 시스템 매개 변수 값에 의해 활성화/비활성화되며, 이 변수 값은 2진법으로 설정한다.`
...
:red:`위에서 트랜잭션 인덱스는 1이고, 프로그램 이름은 csql, 사용자 id은 dba, 호스트 이름은 cubriddb, 클라이언트 프로세스 id는 12854, 격리 수준은 COMMITTED READ, 그리고 잠금 타임아웃은 무제한이다.`
...
:red:`다음 예제는 OID(0|62|5)인 클래스 타입의 객체를 나타내며,트랜잭션 1은 **IX_LOCK**을 보유하고 트랜잭션 2는 **SCH_S_LOCK** 을 보유한 상태에서 **SCH_M_LOCK**으로 변환할 수 없어 차단된 상태를 보여준다.또한 트랜잭션 3은**SCH_S_LOCK** 을 기다리지만 트랜잭션 2가 **SCH_M_LOCK**을 기다리기 때문에 차단된 상태를 보여준다.`
...
:red:`다음 예는 **X_LOCK** 을 보유한 트랜잭션 1에 의해서 삽입된 OID가 ( 2 | 50 | 1)인 클래스의 인스턴스를 보여준다. 트랜잭션1이 삽입한 인스턴스를 수정하기 위해 트랜잭션2가 **X_LOCK** 을 기다리는 것을 보여준다.` 
