...
.. option:: -i, --include-class-file=FILE
     ...
     :red:`-i 옵션을 지정해 복제 불일치를 확인할 테이블을 지정할 수 있다. 테이블을 지정하지 않으면 전체 테이블을 확인한다. 파일 내용의 테이블명 구분자로 사용할 수 있는 기호는 빈 문자열, 탭, 개행문자 및 쉼표이다.`

.. option:: -e, --exclude-class-file=FILE

     -e :red:`옵션을 지정하여 복제 불일치 확인에서 제외할 테이블을 지정할 수 있다. -i와 -e 중 하나만 사용할 수 있다.`
