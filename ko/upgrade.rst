...
다음 표는 http://ftp.cubrid.org/CUBRID_Engine/10.1.0/Linux/에서 배포하는 예약어 검출 스크립트인 check_reserved.sql와 cubrid unloaddb/loaddb 유틸리티를 사용해 마이그레이션을 수행하는 방법을 보여준다. (:ref:`unloaddb` 및 :ref:`loaddb` 참고)
...
*   **log_buffer_size** 의 최소값이 48KB(3*1page, 16KB=1page)에서 2MB(128*1page, 16KB=1page)로 변경되었으므로, 이 값을 설정한 경우 변경된 최소값보다 크게 설정해야 한다.
*   **sort_buffer_size** 의 최대 값이 2G로 제한했으므로 **sort_buffer_size** 의 값은 2G 보다 크게 설정하지 않아야 한다.
*   다음 파리미터 중 기존 파라미터는 더 이상 사용하지 않을 예정이며 신규 파라미터 사용을 권장한다. 괄호 안의 값은 생략될 경우 기본 적용되는 단위이며, 신규 파라미터는 단위 지정이 가능하다. 자세한 내용은 :doc:`/admin/config` 에서 각 파라미터 설명을 참고한다.
