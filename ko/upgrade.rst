...
*   **log_buffer_size** :red:`의 최소 크기가 48KB(3*1page, 16KB=1page)에서 2MB(128*1page, 16KB=1page)로 변경되었므로 이 값은 변경된 최소 크기보다 커야 한다.`
...
*   **log_buffer_size** :red:`의 최소 크기가 48KB(3*1page, 16KB=1page)에서 2MB(128*1page, 16KB=1page)로 변경되었므로 이 값은 변경된 최소 크기보다 커야 한다.`
*   **sort_buffer_size** :red:`의 최대 값이 2G이므로 **sort_buffer_size**의 값은 2G 이하로 설정해야 한다.`
*   :red:`다음 파라미터에서 기존의 파라미터는 사용이 중단될 예정이므로 새 파라미터 사용을 권장한다. 괄호 안의 값은 단위가 생략될 경우 값의 단위를 나타내며, 새 파라미터는 값 다음에 단위를 지정할 수 있다. 자세한 내용은 :doc:`/admin/config`에서 각 파라미터 설명을 참고한다.`
...
*   :red:`멀티바이트 문자를 저장할 공간을 변경할 때는 주의해야 한다. 예를 들어, 2008 R4.3에서는 **CHAR(6)**이 6바이트 크기의 **CHAR** 타입을 의미하지만 9.3부터는 **CHAR(6)**이 6개의 문자의 **CHAR** 타입을 의미한다. utf8 문자 집합에서 한국어는 1개의 문자당 3바이트이므로 **CHAR(6)**은 18바이트이다. 그러므로 디스크 공간이 더 필요하다.`
