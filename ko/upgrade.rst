...
:red:`CUBRID 9.2/9.3/10.0 버전을 사용 중인 사용자는 다른 디렉터리에 10.1을 설치하고 데이터베이스를 10.1로 마이그레이션한 후 이전 환경 설정 파일에서 파라미터 값을 수정해야 한다.`
...
*   **log_buffer_size** :red:`의 최소값이 48KB(3*1page, 16KB=1page)에서 2MB(128*1page, 16KB=1page)로 변경되었으므로, 이 값을 설정한 경우 변경된 최소값보다 크게 설정해야 한다.`
...
:red:`CUBRID 9.1 버전을 사용 중인 사용자는 다른 디렉터리에 10.1을 설치하고 데이터베이스를 10.1로 마이그레이션한 후 이전 환경 설정 파일에서 파라미터 값을 수정해야 한다.`
...
*   **log_buffer_size** :red:`의 최소값이 48KB(3*1page, 16KB=1page)에서 2MB(128*1page, 16KB=1page)로 변경되었으므로, 이 값을 설정한 경우 변경된 최소값보다 크게 설정해야 한다.`
*   **sort_buffer_size** :red:`의 최대 값이 2G로 제한했으므로 **sort_buffer_size** 의 값은 2G 보다 크게 설정하지 않아야 한다.`
*   :red:`다음 파리미터 중 기존 파라미터는 더 이상 사용하지 않을 예정이며 신규 파라미터 사용을 권장한다. 괄호 안의 값은 생략될 경우 기본 적용되는 단위이며, 신규 파라미터는 단위 지정이 가능하다. 자세한 내용은 :doc:`/admin/config` 에서 각 파라미터 설명을 참고한다.`
...
:red:`CUBRID 2008 R4.1, R4.3 또는 R4.4 버전을 사용 중인 사용자는 다른 디렉터리에 10.1을 설치하고 데이터베이스를 10.1로 마이그레이션한 후 기존 환경 설정 파일에서 파라미터 값을 수정해야 한다.`
...
*   :red:`멀티바이트 문자에 대한 저장 공간 변화에 주의해야 한다. 예를 들어 2008 R4.3에서 **CHAR(6)** 은 6바이트 CHAR 타입을 의미하지만 9.3에서 **CHAR(6)** 은 6글자 **CHAR** 타입을 의미한다. utf8 문자셋에서 한글은 한 글자 당 3바이트를 차지하므로, **CHAR(6)** 은 18바이트를 차지한다. 따라서 이전 버전보다 더 많은 디스크 공간을 필요로 한다.`
